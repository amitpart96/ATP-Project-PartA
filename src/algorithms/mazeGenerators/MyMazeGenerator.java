...
public class MyMazeGenerator extends AMazeGenerator {
    ...
    public Maze generate(int r, int c){
        ...
        while(!loop){
            m.reset();
            map = createMaze(m,r,c);
            if (map[m.getGoalPosition().getRowIndex()][m.getGoalPosition().getColumnIndex()].getKashir()==0){
                loop=true;
            }
        }
        return m;
    ...
    public void initBeginCells(Maze m){
        int count = 0;
        Cell c1=map[m.getStartPosition().getRowIndex()][0];
        c1.setKashir(count);
        c1.visit();
        int size = c1.getNumOfNeighbors();
        int choose=rand.nextInt(size);
        Cell c2=c1.getNeighborsByIndex(choose);
        c2.setKashir(count);
        c2.visit();
        c1.removeNeighbors(c2);
        size = c1.getNumOfNeighbors();
        choose=rand.nextInt(size);
        Cell c3=c1.getNeighborsByIndex(choose);
        c3.setKashir(count);
        c3.visit();
